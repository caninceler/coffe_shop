...
- **Proje adı / modül:** `CoffeeShop` (Runtime, C++ modülü mevcut)
...
├── Source/CoffeeShop/       ← C++ modülü (CoffeeShop.Build.cs, .cpp, .h)
├── Content/
...
- C++ Actor sınıfları: `A` öneki (`ACoffeeShopPlayerCharacter`, `ACoffeeMachine`).
...
- Proje sınıfları için tutarlı tema öneki olarak `CoffeeShop` kullan — karışık kullanma.
...
- Müşteri NPC sistemi: Karakter, AI Controller, spawn/service/seat referansları C++ tabanlı; model, animasyon, collision ve Behavior Tree asset atamaları Blueprint üzerinden yapılır. Behavior Tree/Blackboard akışı C++ state ve referanslarını okur.
...
---
*Son güncelleme: 2026-06-09 — runtime C++ modülü `CoffeeShop` olarak adlandırıldı ve proje C++ sınıfları için `CoffeeShop` prefix'i standartlaştırıldı; müşteri NPC mimarisi eklendi; C++ taban + Blueprint görsel/animasyon/collision + Behavior Tree/Blackboard akışı. Önceki kararlar: cozy çekirdek (ceza yok / sıfırlama yok), XP=itibar sistemi, ücretsiz/ücretli malzeme, tuvalet temizliği, şemsiye-gölge XP, içecek istasyonları (süt/kahve/su/şurup/buz/sıcak su/soğuk su), müzik. Bu dosya değiştikçe tarih ve ilgili bölümü güncelleyin.*

Decision 2026-06-09: First-person native interaction skeleton added: ACoffeeShopPlayerCharacter, ACoffeeShopGameMode, UCoffeeShopInteractable; E accepts the next payment at the service point.
